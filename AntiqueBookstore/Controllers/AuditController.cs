...
namespace AntiqueBookstore.Controllers
{
    ...
    public class AuditController : Controller
    {
        ...
        public async Task<IActionResult> ExportAuditLogCsv(/*DateTime? startDate, DateTime? endDate*/)
        {
            // Base query
            ...
            //{
            //    auditLogsQuery = auditLogsQuery.Where(log => log.Timestamp >= startDate.Value);
            //}
            //if (endDate.HasValue)
            //{
            //    // Add 1 day and check for less than to include the whole end date
            //    auditLogsQuery = auditLogsQuery.Where(log => log.Timestamp < endDate.Value.AddDays(1));
            //}
